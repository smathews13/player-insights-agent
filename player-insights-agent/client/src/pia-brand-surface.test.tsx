...
describe('Player Insights Agent visible brand surfaces', () => {
  ...
  it('keeps the theme-aware header lockup collision-safe at desktop and narrow widths', () => {
    ...
    expect(shell).toMatch(
      /\.brand-lockup\s*\{[^}]*width:\s*calc\(var\(--conversation-width\) - var\(--app-header-pad-x\)\)[^}]*min-width:\s*calc\(var\(--conversation-width\) - var\(--app-header-pad-x\)\)[^}]*max-width:\s*calc\(var\(--conversation-width\) - var\(--app-header-pad-x\)\)/s
    );
