...
| 3 | Backend: assembly | `runtime_settings.answer.narrative = false` blanks **both** `narrative` and `content`. Tables are written into `content`. | The answer loses its table. On `tr-3ec6d1c0`, synthesis wrote a 7-row table and the wire carried `content: ""`. | No (the synthesis span has the original) | agent.py:8502-8506, 8546-8547. **Fixed in the backend by MIT-14735**; live once a new model version is served |
